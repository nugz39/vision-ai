...
export const PLACEHOLDER_SRC = FALLBACK_THUMB;

/**
 * Only prompt-01..prompt-12 are valid placeholders.
 * We ONLY reference those here.
 */
export const VISION_GALLERY_ITEMS: GalleryItem[] = [
 ...
 *   styleFromSlug(slug)
 *   styleFromSlug(mode, slug)   // legacy
 */
export function styleFromSlug(a: string, b?: string): AnyStyle {
  const slug = b ?? a;
...
export function getStylesForMode(mode: GalleryMode): AnyStyle[] {
  const styles = Array.from(new Set(VISION_GALLERY_ITEMS.filter(i => i.mode === mode).map(i => i.style)));
  return ["all", ...styles];
}

export function getItemsFor(mode: GalleryMode, style: AnyStyle = "all"): GalleryItem[] {
  const base = VISION_GALLERY_ITEMS.filter(i => i.mode === mode);
  return style === "all" ? base : base.filter(i => i.style === style);
}

/* ---- Legacy type aliases expected by GalleryClient.tsx ---- */
export type VisionMode = GalleryMode;
export type VisionStyle = AnyStyle;
export type VisionQuality = "fast" | "standard" | "ultra";
export type VisionAspect = "1:1" | "4:5" | "16:9";
export type VisionUseCase =
  ...
  | "storyboards"
  | "moodboards";
export type VisionGalleryItem = GalleryItem;